...
if menu == "⛽ Tanken":
    ...
    if c1.button("Speichern ✅"):
        new_row = pd.DataFrame([{
            ...
            "Liter": t_lit, "CHF/L": t_pr, "Total CHF": round(t_lit * t_pr, 2), "Wer": t_wer
        }])
    ...
    if not tank_jahr.empty:
        st.table(tank_jahr)
        tank_jahr["Total CHF"] = pd.to_numeric(tank_jahr["Total CHF"], errors='coerce')
    ...
    <b>Leistung:</b> 425 HP (317 kW) @ 4400-4800 RPM | <b>Hubraum:</b> 8.1 Liter V8<br>
    ...
    st.divider()
    st.metric("TOTAL KOSTEN (inkl. Benzin)", f"CHF {(fix_sum + serv_sum + sprit_sum):,.2f}")
