...
if 'zubehoer' not in st.session_state:
    st.session_state.zubehoer = []
...
with tab1:
    ...
    with col_in:
        with st.container(border=True):
            t_datum = st.date_input("Datum", datetime.now(), key="t_date_v5")
            t_liter = st.number_input("Liter (L)", min_value=0.0, step=10.0, key="t_lit_v5")
            t_preis = st.number_input("CHF / L", value=2.15, key="t_price_v5")
            t_wer = st.radio("Zahler", ["Marc", "Fabienne"], horizontal=True, key="t_who_v5")
            
            c_btn1, c_btn2 = st.columns(2)
            with c_btn1:
                if st.button("Speichern ✅", use_container_width=True, key="btn_save_t"):
                    if t_liter > 0:
                        ...
                        st.rerun()
            with c_btn2:
                if st.button("Letzten löschen 🗑️", use_container_width=True, key="btn_del_t"):
...
with tab2:
    ...
    with col_m2:
        ...
        up = st.file_uploader("Rechnung hochladen", type=['jpg', 'jpeg', 'png'], key="f_up_v5")
...
with tab3:
    ...
    with col_s1:
        ...
        s_text = st.text_area("Arbeiten", key="s_txt_v5")
        if st.button("Service speichern", key="btn_s_save"): st.success("Eintrag gemerkt")
    with col_s2:
        st.write("### Zubehör")
        zub_n = st.text_input("Teil", key="z_n_v5")
        zub_p = st.number_input("Preis CHF", min_value=0.0, key="z_p_v5")
        if st.button("Zubehör speichern", key="btn_z_save"):
...
with tab4:
    ...
    with col_k1:
        ...
        k_axa = st.number_input("AXA Versicherung 🛡️", value=1150.0, key="k_axa_v5")
        k_platz = st.number_input("Bootsplatz ⚓", value=1500.0, key="k_platz_v5")
        k_winter = st.number_input("Winterlager ❄️", value=2200.0, key="k_winter_v5")
        k_steuer = st.number_input("Steuern 📜", value=350.0, key="k_tax_v5")
...
with tab5:
    ...
    st.text_input("Törn-Ziel", key="l_dest_v5")

st.write("---")
st.caption("Truelove Fleet v19.5 | Stable Build")
