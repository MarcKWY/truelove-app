...
function doPost(e) {
  try {
    ...
    if (!sheet) return ContentService.createTextOutput("Sheet nicht gefunden").setMimeType(ContentService.MimeType.TEXT);

    if (data.method === "append") {
      // Speichert Daten ans Ende
      sheet.appendRow(data.values);
      return ContentService.createTextOutput("Erfolgreich hinzugefügt").setMimeType(ContentService.MimeType.TEXT);
    } 
    
    else if (data.method === "update") {
      // Überschreibt Fixkosten in Zeile 2 (Spalte A-D)
      sheet.getRange(2, 1, 1, data.values.length).setValues([data.values]);
      return ContentService.createTextOutput("Fixkosten aktualisiert").setMimeType(ContentService.MimeType.TEXT);
    }

    else if (data.method === "delete") {
      // Löscht Zeile (Index + 2, da Sheets bei 1 startet und Kopfzeile existiert)
      sheet.deleteRow(data.index + 2);
      return ContentService.createTextOutput("Gelöscht").setMimeType(ContentService.MimeType.TEXT);
    }
