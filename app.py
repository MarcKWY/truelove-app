...
st.markdown("""
    ...
    /* Blauer Balken Optimierung: Weisse Schrift, fett und schattiert für beste Lesbarkeit */
    .total-box { 
    ...
    .total-box h1, .total-box h3 { color: #FFFFFF !important; margin: 0; padding: 5px; text-shadow: 1px 1px 2px rgba(0,0,0,0.3); }
...
with tab1:
    ...
    with col_in:
        with st.container(border=True):
            t_datum = st.date_input("Datum", datetime.now(), key="t_date_v6")
            t_liter = st.number_input("Liter (L)", min_value=0.0, step=10.0, key="t_lit_v6")
            t_preis = st.number_input("CHF / L", value=2.15, key="t_price_v6")
            t_wer = st.radio("Zahler", ["Marc", "Fabienne"], horizontal=True, key="t_who_v6")
            c_btn1, c_btn2 = st.columns(2)
            with c_btn1:
                if st.button("Speichern ✅", use_container_width=True, key="btn_save_t_v6"):
                    if t_liter > 0:
                        st.session_state.tank_daten.append({"Datum": t_datum.strftime("%d.%m.%Y"), "Liter": t_liter, "Total": round(t_liter * t_preis, 2), "Zahler": t_wer})
                        st.rerun()
            with c_btn2:
                if st.button("Letzten löschen 🗑️", use_container_width=True, key="btn_del_t_v6"):
...
# TAB 2: MOTOR & SERVICE (Zusammengelegt)
with tab2:
    st.subheader("⚙️ Motor-Daten & Service-Historie")
    col_m1, col_m2 = st.columns(2)
    with col_m1:
        st.markdown("""<div class="spec-card"><h3>Mercruiser 496 MAG HO</h3><ul><li><b>Leistung:</b> 317 kW / 431 PS</li><li><b>Hubraum:</b> 8.2 Liter</li><li><b>Zylinder:</b> V8 Big Block</li><li><b>Kühlung:</b> Zweikreissystem</li><li><b>WOT:</b> 4600-5000 RPM</li><li><b>Ölkapazität:</b> 8.5 L</li></ul></div>""", unsafe_allow_html=True)
        if os.path.exists("motor.jpg"): st.image("motor.jpg", use_container_width=True)
    with col_m2:
        with st.container(border=True):
            st.write("### 🔧 Service & Reparatur eintragen")
            s_datum = st.date_input("Datum", datetime.now(), key="s_date_v6")
            s_arbeit = st.text_input("Was wurde gemacht?", key="s_work_v6")
            s_chf = st.number_input("Kosten Service (CHF) 🛠️", min_value=0.0, key="s_price_v6")
            if st.button("Service/Reparatur speichern", key="btn_s_save_v6"):
                st.session_state.service_kosten.append({"Datum": s_datum.strftime("%d.%m.%Y"), "Arbeit": s_arbeit, "CHF": s_chf})
                st.rerun()
        
        st.write("---")
        st.write("### 📂 Dokumente & Verlauf")
        up = st.file_uploader("Rechnung hochladen", type=['jpg', 'jpeg', 'png'], key="f_up_v6")
...
# TAB 3: KOSTEN
with tab4: # Interner Key tab4 beibehalten für Stabilität
    st.subheader("💰 Gesamtkosten Truelove")
    col_k1, col_k2 = st.columns(2)
    with col_k1:
        st.image("https://wikimedia.org", width=50)
        k_axa = st.number_input("AXA Versicherung 🛡️", value=1150.0, key="k_axa_v6")
        k_platz = st.number_input("Bootsplatz ⚓", value=1500.0, key="k_platz_v6")
        k_winter = st.number_input("Winterlager ❄️", value=2200.0, key="k_winter_v6")
        k_steuer = st.number_input("Steuern 📜", value=350.0, key="k_tax_v6")
    ...
    with col_k2:
        ...
        st.markdown(f"""
            <div class='total-box'>
                <h3>GESAMTKOSTEN (Inkl. Benzin)</h3>
                <h1>CHF {total_incl:,.2f}</h1>
            </div>
        """, unsafe_allow_html=True)

# TAB 4: LOGBUCH
with tab5: # Interner Key tab5 beibehalten
    st.header("📖 Fahrtenbuch")
    if os.path.exists("boot_gross.jpg"): st.image("boot_gross.jpg", width=400)
    st.text_input("Törn-Ziel", key="l_dest_v6")

st.write("---")
st.caption("Truelove Fleet v19.6 | Master Build")
