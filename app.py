...
st.markdown("""
    ...
    /* Blauer Balken mit extrem guter Lesbarkeit */
    .total-box { 
...
# --- REITER-STRUKTUR (Jetzt korrekt auf 4 Reiter begrenzt) ---
...
with tab1:
    ...
    with col_in:
        with st.container(border=True):
            t_datum = st.date_input("Datum", datetime.now(), key="t_date_v7")
            t_liter = st.number_input("Liter (L)", min_value=0.0, step=10.0, key="t_lit_v7")
            t_preis = st.number_input("CHF / L", value=2.15, key="t_price_v7")
            t_wer = st.radio("Zahler", ["Marc", "Fabienne"], horizontal=True, key="t_who_v7")
            c_btn1, c_btn2 = st.columns(2)
            with c_btn1:
                if st.button("Speichern ✅", use_container_width=True, key="btn_save_t_v7"):
                    if t_liter > 0:
                        st.session_state.tank_daten.append({"Datum": t_datum.strftime("%d.%m.%Y"), "Liter": t_liter, "Total": round(t_liter * t_preis, 2), "Zahler": t_wer})
                        st.rerun()
            with c_btn2:
                if st.button("Letzten löschen 🗑️", use_container_width=True, key="btn_del_t_v7"):
...
with tab2:
    ...
    with col_m2:
        with st.container(border=True):
            ...
            s_datum = st.date_input("Datum", datetime.now(), key="s_date_v7")
            s_arbeit = st.text_input("Was wurde gemacht?", key="s_work_v7")
            s_chf = st.number_input("Kosten (CHF)", min_value=0.0, key="s_price_v7")
            if st.button("Eintrag speichern", use_container_width=True, key="btn_s_save_v7"):
                st.session_state.service_kosten.append({"Datum": s_datum.strftime("%d.%m.%Y"), "Arbeit": s_arbeit, "CHF": s_chf})
                st.rerun()
        st.write("---")
        up = st.file_uploader("Rechnung hochladen", type=['jpg', 'jpeg', 'png'], key="f_up_v7")
...
with tab3:
    ...
    with col_k1:
        ...
        k_axa = st.number_input("AXA Versicherung 🛡️", value=1150.0, key="k_axa_v7")
        k_platz = st.number_input("Bootsplatz ⚓", value=1500.0, key="k_platz_v7")
        k_winter = st.number_input("Winterlager ❄️", value=2200.0, key="k_winter_v7")
        k_steuer = st.number_input("Steuern 📜", value=350.0, key="k_tax_v7")
...
with tab4:
    ...
    st.text_input("Törn-Ziel", key="l_dest_v7")

st.write("---")
st.caption("Truelove Fleet v19.7 | Fixed Build")
